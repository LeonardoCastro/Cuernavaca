...
jldopen("S2-ConDatos-Loc.jld", "w") do file
  for  (num_j, j) in enumerate(J_in)
      ...
      for n = 1:N
          ...
          Sense_2 = Highway.Highway2D(3, 3640+6)
          S2 = Sense_2.highway
          ...
          for t = 0:Tf-1
                ...
                if rand() < j
                  Insert_Vehicle!(S2[1].highway, 3t, f*P_ramp_S2[1][2]/Lanes)
                end
                if rand() < j
                  Insert_Vehicle!(S2[2].highway, 3t+1, f*P_ramp_S2[1][2]/Lanes)
                end
                if rand() < j
                  Insert_Vehicle!(S2[3].highway, 3t+2, f*P_ramp_S2[1][2]/Lanes)
                end

                for (num_ramp, (x0, lramp)) in enumerate(S2_in_ramp)
                    Ramp!(x0, lramp, f*P_ramp_S2[1+num_ramp][1], f*P_ramp_S2[1+num_ramp][2], S2[1], 1)
                    num_ramp = x0 = lramp = 0
                end
                ...
                for k = 1:Lanes
                  ...
                  if t > t1
                      Measure_Frequencies!(S2[k].highway, div(t-t1+1, T)+1, Sections, T,
